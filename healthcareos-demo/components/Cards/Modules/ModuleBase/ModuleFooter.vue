<template>
  <div
    class="module-footer flex flex-row w-full px-4 pb-4 pt-4 justify-between bg-white/20">
    <div class="flex flex-row gap-x-3">
      <h-button
        :to="primaryButton.path"
        variant="success"
        >{{ primaryButton.text }}</h-button
      >
      <NuxtLink
    ...
    <div v-if="tertiaryButton != null">
      <h-button
        :to="tertiaryButton.path"
        v-on:click="$emit('tertiary-button-action', tertiaryButton.value)">
        {{ tertiaryButton.text }}
      </h-button>
    </div>
  </div>
